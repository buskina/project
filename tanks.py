...
class Tank2(pygame.sprite.Sprite):
    def __init__(self, screen):
        ...
        tank2_img = pygame.image.load(
            path.join(img_dir, "tank2.png")).convert()
...
class Player(pygame.sprite.Sprite):
    def __init__(self, screen):
        ...
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.Surface((50, 40))
        tank1_img = pygame.image.load(
            path.join(img_dir, "tank1.png")).convert()
        self.a = 100
    ...
    def targetting(self, x1, y1):
        ...
        if (x1-self.rect.centerx) > 0:
            self.bn = math.atan((-y1+self.rect.centery) /
                                (x1-self.rect.centerx))
        if (x1-self.rect.centerx) < 0:
            self.bn = 180 + \
                math.atan((-y1+self.rect.centery) / (x1-self.rect.centerx))
    ...
    def gun(self):
        ...
        self.L += self.f2_power / \
            100  # длина ствола зависит от времени удержания.
        self.xo = self.rect.centerx+self.a+self.L * \
            math.cos(self.bn)  # конец ствола по х
        self.yo = -self.L*math.sin(self.bn) + \
            self.rect.centery-self.b  # конец ствола по y
        # рисует ствол пушки
...
class Targ1(pygame.sprite.Sprite):
    def __init__(self):
        ...
        targ1_img = pygame.image.load(
            path.join(img_dir, "targ1.png")).convert()
